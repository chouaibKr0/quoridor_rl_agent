...
class QuoridorUI:
    ...
    def handle_move_click(self, pos):
        ...
        # Calculate delta and find action
        dr = target[0] - curr[0]
        dc = target[1] - curr[1]
    ...
    def update_wall_hover(self, pos):
        ...
        # Wall intersections are at cell boundaries
        if self.wall_orientation == 'h':
            # Horizontal walls are between rows
            # Snap to nearest horizontal wall position
            row = round(rel_y / self.cell_size) - 1
            col = int(rel_x / self.cell_size)
            
            # Clamp to valid range (0-7 for 8x8 wall grid)
            row = max(0, min(7, row))
            col = max(0, min(7, col))
            
            self.hovered_wall = ('h', row, col)
        else:
            # Vertical walls are between columns
            # Snap to nearest vertical wall position
    ...
    def draw_board(self):
        ...
        for i in range(10):
            ...
            pygame.draw.line(self.screen, self.GRID_COLOR, 
                           (x, self.margin), 
                           (x, self.margin + self.board_size), 2)
            
            # Horizontal lines
            y = self.margin + i * self.cell_size
            pygame.draw.line(self.screen, self.GRID_COLOR,
                           (self.margin, y),
                           (self.margin + self.board_size, y), 2)
    ...
    def draw_walls(self):
        ...
        for r, c in self.game.walls_h:
            ...
            pygame.draw.rect(self.screen, self.WALL_COLOR,
                           (x, y - 6, self.cell_size * 2, 12))
            # Add border for visibility
            pygame.draw.rect(self.screen, (200, 200, 200),
                           (x, y - 6, self.cell_size * 2, 12), 1)
        ...
        for r, c in self.game.walls_v:
            ...
            pygame.draw.rect(self.screen, self.WALL_COLOR,
                           (x - 6, y, 12, self.cell_size * 2))
            # Add border for visibility
            pygame.draw.rect(self.screen, (200, 200, 200),
                           (x - 6, y, 12, self.cell_size * 2), 1)
    ...
    def draw_wall_preview(self):
        ...
        if wall_type == 'h':
            # Horizontal wall preview
            x = self.margin + c * self.cell_size
            y = self.margin + (r + 1) * self.cell_size
            # Draw thicker preview
            pygame.draw.rect(self.screen, color,
                           (x, y - 6, self.cell_size * 2, 12))
            # Add outline
            pygame.draw.rect(self.screen, (255, 255, 255),
                           (x, y - 6, self.cell_size * 2, 12), 2)
        else:
            # Vertical wall preview
            x = self.margin + (c + 1) * self.cell_size
            y = self.margin + r * self.cell_size
            # Draw thicker preview
            pygame.draw.rect(self.screen, color,
    ...
    def draw_info_panel(self):
        ...
        # Title
        title = self.font.render("QUORIDOR", True, self.TEXT_COLOR)
